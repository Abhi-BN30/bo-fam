...
export async function POST(req: Request) {
  const { primary_name, spouse_name, primary_email, dob, parent_id, contact, spouse_email, address, city, state, country, spouse_contact } = await req.json();
  const res = await sql`INSERT INTO users (primary_name, spouse_name, primary_email, dob, contact, spouse_email, address, city, state, country, spouse_contact) 
    VALUES (${primary_name}, ${spouse_name}, ${primary_email}, ${dob}, ${contact}, ${spouse_email}, ${spouse_contact} ${address || null}, ${city || null}, ${state || null}, ${country || null}) RETURNING id`;
